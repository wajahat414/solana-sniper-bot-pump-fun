...
async function get_price_from_transaction(mint: String): Promise<number> {
  ...
  await axios
    ...
    .then((response) => {
      ...
      const nativeBalanceChangeLamports = jsonData[0].accountData.find(
      ...
      if (nativeBalanceChangeLamports && tokenAmountTransferred) {
        const nativeBalanceChangeSol =
          nativeBalanceChangeLamports / 1_000_000_000;
        const rate_up = nativeBalanceChangeSol / tokenAmountTransferred;
        console.log(
          `tokenAmountTransferred: ${tokenAmountTransferred}, nativeBalanceChangeSol: ${nativeBalanceChangeSol}`
        );
...
async function get_data_from_transaction(signature: String): Promise<any> {
  ...
  let config = {
    method: "post",
    maxBodyLength: Infinity,
    url: "https://api.helius.xyz/v0/transactions?api-key=ee3680cf-196e-475b-8f62-812d6b540435",
    headers: {
  ...
  await axios
    ...
    .then((response) => {
      ...
      if (nativeBalanceChangeLamports && tokenAmountTransferred) {
        nativeBalanceChangeSol = nativeBalanceChangeLamports / 1_000_000_000;
        const rate_up = nativeBalanceChangeSol / tokenAmountTransferred;
        console.log(
          `tokenAmountTransferred: ${tokenAmountTransferred}, nativeBalanceChangeSol: ${nativeBalanceChangeSol}`
        );
      ...
      const data = {
        nativeBalanceChanges: nativeBalanceChangeSol,
        tokenTransfers: tokenAmountTransferred,
        rate: rate,
      };
...
export { get_price_from_transaction, get_data_from_transaction };
