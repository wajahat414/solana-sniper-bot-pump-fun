...
start();
 ...
 async function subscribeAppEvents () {
    PubSub.subscribe(EventType.GET_NEW_MINT, (_, payload) => handleGetNewMint(payload));

    PubSub.subscribe(EventType.CREATE_ASSOCIATED_TOKEN_ACCOUNT, (_, payload) => handleCreateAssociatedTokenAccount(payload));
    PubSub.subscribe(EventType.GET_INTITIAL_PRICE, (_, payload) => handleGetInitialTokenPrice(payload));
    PubSub.subscribe(EventType.SETUP_BUY_TRADE, (_, payload) => handleSetupBuyTrade(payload));
    PubSub.subscribe(EventType.EXECUTE_BUY_TRADE, (_, payload) => handleExecuteBuyTrade(payload));
 }
...
async function handleGetNewMint(payload: any): Promise<void> {
    console.log('get_new_mint in subscribeAppEvents');

    const  data = await getTransactions();

    if (data!=AppCodes.FAILED && data!=AppCodes.FAILED_FETCHING_TRANSACTIONS ) {
      ...
      if (resp == AppCodes.SUCCESS) {
         ...
         PubSub.publish(EventType.CREATE_ASSOCIATED_TOKEN_ACCOUNT,{});
          ...
          Helpers.showAlert('Error', `Failed to get token data from transactions ${resp}`);
      }
